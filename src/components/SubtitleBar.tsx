...
function findSubtitle(subtitles: SubtitleItem[], offset: number): SubtitleItem | null {
  return subtitles.find(s => offset >= s.charStart && offset < s.charEnd) ?? null;
}

export function SubtitleBar({ subtitles, activeOffset, visible, loading = false }: Props) {
  ...
  useEffect(() => {
    ...
    const found = activeOffset !== null ? findSubtitle(subtitles, activeOffset) : null;
